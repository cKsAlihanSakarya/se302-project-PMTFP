...
import { getAdvisorRequests, updateAdvisorRequest, getInstructorProfile, updateInstructorProfile, getAdvisingProjects } from '../services/api';
...
function InstructorDashboard() {
    ...
    const [advisingProjects, setAdvisingProjects] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const projectsRes = await getAdvisingProjects();
                setAdvisingProjects(projectsRes.data);
            } catch (err) {
                console.error(err);
            }
        };
                ...
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                                ...
                                <div key={request.id} className="bg-white border border-gray-200 rounded-xl p-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex flex-col items-end gap-2">
                                            <span className={`text-xs px-3 py-1 rounded-full ${request.status === 'pending' ? 'bg-amber-50 text-amber-700' :
                            ...
                            {/* Advising Projects */}
                            <div className="mt-6">
                                <h3 className="font-semibold text-gray-700 mb-4">Projects I'm Advising</h3>
                                {advisingProjects.length === 0 && (
                                    <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
                                        <p className="text-gray-400 text-sm">No advising projects yet.</p>
                                    </div>
                                )}
                                <div className="space-y-3">
                                    {advisingProjects.map(project => (
                                        <div key={project.id} className="bg-white border border-gray-200 rounded-xl p-4">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <h4 className="font-semibold text-gray-800 text-sm mb-1">{project.title}</h4>
                                                    <p className="text-gray-500 text-xs mb-1">Owner: {project.owner_name}</p>
                                                    <span className={`text-xs px-2 py-0.5 rounded-full ${project.project_type === 'tubitak' ? 'bg-teal-50 text-teal-700' :
                                                        project.project_type === 'teknofest' ? 'bg-purple-50 text-purple-700' :
                                                            'bg-amber-50 text-amber-700'
                                                        }`}>{project.project_type}</span>
                                                </div>
                                                <span className="text-xs px-3 py-1 rounded-full bg-green-50 text-green-700">Advising</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
